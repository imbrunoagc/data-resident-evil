...
class PandasBucket:
    ...
    def read_json_from_minio(self, name: str) -> List:
        data_io = self.__get__(name)  # Usa o método __get__ para obter o objeto do MinIO
        data = json.load(data_io)  # Carrega os dados JSON a partir do objeto BytesIO
        return data
    ...
    def write_dict_to_json(
        ...
        **kwargs
        ...
        data_io = io.BytesIO()
        data_json = json.dumps(data, indent=4) # Usa dumps para obter a string JSON
        data_json = data_json.encode('utf-8') # Encode para bytes
        data_io.write(data_json)
        nb_bytes = data_io.tell()
        data_io.seek(0)
        return self.__put__(name, length=nb_bytes, data=data_io)