...
class PandasBucket:
    ...
    def destroy(self) -> None:
        ...
        if self.exists:
            for obj in self.client.list_objects_v2(Bucket=self.name).get(
                'Contents', []
            ):
    ...
    def put_feather(
        self, df: Union[pd.Series, pd.DataFrame], name: str
    ) -> str:
    ...
    def put_json(
        self, df: Union[pd.Series, pd.DataFrame], name: str, **kwargs
    ) -> str:
