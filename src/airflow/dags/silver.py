...
class ResidentEvil_Bronze_to_Silver:
    def __init__(self) -> None:
        self.client = boto3.client(
            ...
            endpoint_url=os.environ.get(
                'ENDPOINT'
            ),  #'http://localhost:9000'  # Correct API port
    ...
    def _transform_json_to_dataframe(self) -> pd.DataFrame:
        ...
        df['ano_de_nascimento'] = (
            df['Ano de nascimento']
            .fillna(df['de nascimento'])
            .apply(extract_year)
        )
        df['tipo_sanguineo'] = df['Tipo sanguíneo'].apply(
            extract_type_sanguine
        )
        df['altura'] = df['Altura'].apply(extract_height)
        df['peso'] = df['Peso'].apply(extract_weight)
        df['altura'] = df['altura'].apply(
            extract_number
        )  # extração apenas do valor numerico
        df['peso'] = df['peso'].apply(
            extract_number
        )  # extração apenas do valor numerico
        df['pathimage'] = (
            'https://www.residentevildatabase.com/wp-content/uploads/2023/12/'
            + df['name']
            + '.jpg'
        )
